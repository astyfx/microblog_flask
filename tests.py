...
class TestCase(unittest.TestCase):
	...
	def test_follow(self):
		u1 = User(nickname = 'astyfx', email = 'astyfx@gmail.com')
		u2 = User(nickname = 'asty_aol', email = 'astyfx@aol.com')
		db.session.add(u1)
		db.session.add(u2)
		db.session.commit()
		...
		assert u1.followed.first().nickname == 'asty_aol'
		assert u2.followers.count() == 1
		assert u2.followers.first().nickname == 'astyfx'
		u = u1.unfollow(u2)
		assert u != None
		db.session.add(u)
		db.session.commit()
